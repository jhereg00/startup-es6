...
class GLTexture2d {
    ...
    // make and bind the texture
		...
		this._gl.texParameteri(this._gl.TEXTURE_2D, this._gl.TEXTURE_WRAP_S, settings.wrap);
		this._gl.texParameteri(this._gl.TEXTURE_2D, this._gl.TEXTURE_WRAP_T, settings.wrap);
		this._gl.texParameteri(this._gl.TEXTURE_2D, this._gl.TEXTURE_MIN_FILTER, settings.minFilter);
		this._gl.texParameteri(this._gl.TEXTURE_2D, this._gl.TEXTURE_MAG_FILTER, settings.magFilter);
	...
	setSrc (src) {
		...
		this.bind();
		if (src === undefined || src === null) {
			return this._gl.texImage2D(this._gl.TEXTURE_2D, 0, this.settings.format, this.settings.width, this.settings.height, 0, this.settings.format, this.settings.dataType, null);
		}
		else {
			// we have an element, because we died before now if we didn't
			return this._gl.texImage2D(this._gl.TEXTURE_2D, 0, this.settings.format, this.settings.format, this.settings.dataType, src);
		}
	}
}
